$.extend(Tuli.mods,{
	...
	modHeadNav:function(){
				...
				$.post('/login/?number='+Math.random(),{username:username,password:password,auto_login:auto_login,client_type:type},function(data){
					...
					if(source.ok == false){
						...
						switch (type){
							case '1':url = window.location.href;
									break;
							case '2':url = ( urlname == null )?'http://www.'+Tuli.realmName+'/user/'+id+'/':'http://'+urlname+'.'+Tuli.realmName+'/';
									break;
							case '3':url = ( urlname == null )? 'http://www.'+Tuli.realmName+'/friend/albums/':'http://'+urlname+'.'+Tuli.realmName+'/friend/albums/';
									break;
							case '4':
									$('#Header .refresh').show();
									$('#Tuli_log,#Header .User .Log_btn,.dialogIE,.QfeedOut').hide();
		...
		//新手提示
		var firstHover = 0;
		if(loadPageType=='http://www.'+Tuli.realmName+'/all/'||loadPageType=='http://www.'+Tuli.realmName+'.com/'){
			loadPageType = true;
			firstHover = 1;
		};
		var datum = '';
				...
				if($('#Header .userImg img').length == 0){
					...
					if(KW_Config.check_authenticated()){
						...
						$.post('/login/client/kw/',{'token':token},function(data) {
							var source = JSON.parse(data);
							if(source.ok == true){
								window.location.href ="http://www."+Tuli.realmName+"/?client=kw";
							}
						});
					}
				}
	...
	modIndex:function(){
		...
		//QQ通信
		function starQQ(qq) {
			this.link = "tencent://message/?uin="+qq+"&Site=tuli.com&Menu=yes";
			...
			$.post(url,{album_id:id,random:Math.random()},function(data){
				var source = JSON.parse(data);
					if(source.ok == false){
						...
						//图集名称
						if(kktv){
							var title = '<div class="Pop_Title"><a class="statc_album" href="/static/album/'+a_id+'/" target="_blank">'+source.data.album_name+'</a><a href="'+source.data.relation_info.value+'" class="toKKTV" title="穿越到美女KKTV直播间" target="_blank"><img title="穿越到美女KKTV直播间" src="/images/KKTV_black.gif"></a></div>';
						}else{
			...
			$.get('/recommend/album/',function(data){
				...
				if(source.ok == true){
					...
					for(var  i=0;i<len;i++){
						...
						sImgList+="</div></div><a href='#' class='Save_btn'>收藏美女</a></div>";
						htmlData +=sImgList;
					}
